<script lang="ts">
	...
	export function square_updateRaisedColumn() {
		...
		if ($tfield.raised == "odd") {
			for (let col = 1; col < $tfield.columns; col += 2) {
				...
				let newHexCoords: cube_coords = coords_qToCube(
					"odd",
					col,
					$tfield.rows - 1,
				);
				let newId: hex_id =
					genHexId_coordsObj(newHexCoords);
				$tfield.hexes[newId] = {
					q: newHexCoords.q,
					r: newHexCoords.r,
					s: newHexCoords.s,
				...
				for (
					let row = $tfield.rows - 1;
					row >= 0;
					row--
				) {
					let destinationCoords = coords_qToCube(
						"odd",
						col,
						row,
					);
					...
					comp_coordsLayer.updateCoordText(
						genHexId_coordsObj(
							destinationCoords,
						),
					);
				...
				let oldHexCoords = coords_qToCube(
					"even",
					col,
					0,
				);
				...
				delete $tfield.hexes[
					genHexId_coordsObj(oldHexCoords)
				];
				comp_coordsLayer.eliminateCoord(
					genHexId_coordsObj(oldHexCoords),
				);
			}
			...
			for (let col = 1; col < $tfield.columns; col += 2) {
				...
				let newHexCoords = coords_qToCube(
					"even",
					col,
					0,
				);
	...
	export function square_changeIndentedRow() {
		if ($tfield.raised == "odd") {
			for (let row = 1; row < $tfield.rows; row += 2) {
				...
				let newHexCoords = coords_rToCube(
					"odd",
					$tfield.columns - 1,
					row,
				);
				...
				for (
					let col = $tfield.columns - 1;
					col >= 0;
					col--
				) {
					let destinationCoords = coords_rToCube(
						"odd",
						col,
						row,
					);
				...
				let oldHexCoords = coords_rToCube(
					"even",
					0,
					row,
				);
	...
	function square_changeOrientation() {
		...
		for (let col = 0; col < $tfield.columns; col++) {
			for (let row = 0; row < $tfield.rows; row++) {
				...
				newHexes[genHexId_coordsObj(newHexCoords)] =
					newHex;
			}
		}
	...
	function square_expandRight(amount: number) {
		for (let col = 0; col < amount; col++) {
			for (let row = 0; row < $tfield.rows; row++) {
				...
				comp_coordsLayer.generateNewCoord(
					genHexId_coordsObj(newHexCoords),
				);
			}
	...
	function square_moveAllHexesRight(amount: number) {
		for (let col = $tfield.columns - 1; col >= amount; col--) {
			for (let row = 0; row < $tfield.rows; row++) {
				...
				let sourceHex =
					$tfield.hexes[
						genHexId_coordsObj(sourceCoords)
					];
	...
	export function square_reduceMapDimension(
		...
		switch (direction) {
			case "right":
				if (amount >= $tfield.columns)
					amount = $tfield.columns - 1;
				if (amount == 0) return;
			...
			case "bottom":
				if (amount >= $tfield.rows)
					amount = $tfield.rows - 1;
				if (amount == 0) return;
	...
	function square_removeRight(amount: number) {
		for (let col = 0; col < amount; col++) {
			for (let row = 0; row < $tfield.rows; row++) {
				...
				eliminateHex(
					genHexId_coordsObj(fatedHexCoords),
				);
			}
	...
	export function areAllHexesBlank(): boolean {
		return (
			null ==
			Object.entries($tfield.hexes).find(
				([id, hex]) => hex.tile != null,
			)
		);
	}
	...
	export function removeAllTilesOfSet(setId: string) {
		...
		Object.entries($tfield.hexes).forEach(
			([hexId, hex]: [hex_id, TerrainHex]) => {
				if (!hex.tile) return;
	...
	function paintbucket() {
		...
		getContiguousHexIdsOfSameType(clickedId).forEach(
			(hexId: hex_id) => {
	...
	export function get_existant_neighbours(hex_id: hex_id): TerrainHex[] {
		...
		let valid_neighbour_ids = neighbourIds.filter((id) =>
			hexExists(id),
		);
	...
	function getContiguousHexIdsOfSameType(hexId: hex_id): hex_id[] {
		...
		while (hexStack.length > 0) {
			...
			let neighbourIds = getNeighbours(
				currentHex.q,
				currentHex.r,
				currentHex.s,
			);
	...
	export function pointerdown() {
		if ($data_terrain.usingEyedropper) {
			...
			$data_terrain.usingPaintbucket &&
			$data_terrain.usingEraser
		) {
			erasePaintbucket();
