<script lang="ts">
	...
	export let cont_bighex_overlay;
	let grph_overlay = new PIXI.Graphics();

	$: { 
		grph_overlay.visible = tfield.overlay.shown
	}
	...
	afterUpdate(() => {
		...
		grph_overlay.lineStyle(tfield.overlay.style.width, tfield.overlay.style.color);
		...
		for (let bigRow = 0 - encompassMod; bigRow < bigRows + encompassMod; bigRow++) {
			for (let bigCol = 0 - encompassMod; bigCol < bigCols + encompassMod; bigCol++) {
				...
				grph_overlay.drawPolygon(bigHexPath);
			}
		}
	...
	onMount(() => {
		cont_bighex_overlay.removeChildren(0);

		grph_overlay = new PIXI.Graphics();
		grph_overlay.visible = tfield.overlay.shown
		cont_bighex_overlay.addChild(grph_overlay);
	})
</script>
