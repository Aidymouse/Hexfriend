...
function generateTerrain(tfield: TerrainField, tileset: Tileset) {
  let addedNeighbours = []
  ...
  while (addedNeighbours.length != 0) {
    ...
    hexNeighbours.forEach((nId) => {
      if (tfield.hexes[nId] != undefined) {
        if (
          coloredHexes.find((c) => c == nId) == undefined &&
          addedNeighbours.find((c) => c.hexId == nId) == undefined
        ) {
