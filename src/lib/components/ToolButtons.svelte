<script lang="ts">
	...
	export let selected_tool: string;
	export let hex_orientation: 'flatTop' | 'pointyTop';
...
<main>
	{#each buttons as b}
		<div class="tool-button-container">
			...
			<button
				class:selected={selected_tool == b.toolCode}
				on:click={() => { changeTool(b.toolCode) }}
				title={`${b.display} Tool`}
			>
				<!-- Button Image -->
				{#if b.toolCode == 'terrain'}
					<img src={`/assets/img/tools/${b.toolCode}_${hex_orientation == 'flatTop' ? 'ft' : 'pt'}.png`} alt={`${b.display} Tool`} />
				{:else}
			...
			<!-- Mini Buttons -->
			{#if selected_tool == b.toolCode && b.miniButtons.length > 0}
