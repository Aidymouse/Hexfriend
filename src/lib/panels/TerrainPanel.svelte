<script lang="ts">
    ...
    function findSymbolScale(symbolData) {
        if (tfield.hexWidth < tfield.hexHeight) {
            let s = tfield.hexWidth * symbolData.percentOfTotalHex / 100 / symbolData.texWidth;
            return {
                x: s,
                y: s
            }
            ...
            let s = tfield.hexHeight * symbolData.percentOfTotalHex / 100 / symbolData.texHeight;
            return {
                x: s,
                y: s
            }
...
<div class="panel">
    <div id="terrain-preview">
        ...
        <ColorInputPixi bind:value={data_terrain.bgColor} label={"Background"} />

        {#if data_terrain.symbolData}
            <ColorInputPixi bind:value={data_terrain.symbolData.color} label={"Symbol"} />
        {/if}
