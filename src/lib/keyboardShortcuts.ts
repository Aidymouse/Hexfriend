...
export let shortcuts: shortcut_data[] = [
  ...
  { keycode: 'shift', function: null, tool: tools.TERRAIN, display: 'Erase', displayKeycode: 'shift (Hold)' },
  {
    ...
    function: null,
    tool: tools.TERRAIN,
    display: 'Paint Bucket',
    displayKeycode: 'control (Hold)',
  },
  ...
  // ERASER
  {
    ...
    function: null,
    tool: tools.ERASER,
    display: 'Only Erase Terrain',
    displayKeycode: 'shift (Hold)',
  },
  {
    keycode: 'control',
    function: null,
    tool: tools.ERASER,
    display: 'Only Erase Icons',
    displayKeycode: 'control (Hold)',
  },
...
export function getKeyboardShortcut(keyCode: string, $store_selected_tool: tools) {
  return shortcuts.find(
    (shortcut: shortcut_data) =>
      shortcut.keycode == keyCode && (shortcut.tool == $store_selected_tool || shortcut.tool == null),
  )
}
