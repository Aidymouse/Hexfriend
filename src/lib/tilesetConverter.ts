...
const convert_v3_to_v4 = async (old: Tileset): Promise<Tileset> => {
  ...
  for (const tile of t.tiles) {
    ...
    const previews = await generate_tile_previews(
      tile,
      { hexWidth: 50, hexHeight: 43.3, orientation: HexOrientation.FLATTOP, color: DEFAULT_BLANK_HEX_COLOR_STRING },
      spr,
      grph,
      cont,
      app,
...
export const convert_tileset_to_latest = async (old: Tileset) => {
  ...
  if (!new_tileset.format_version || new_tileset.format_version === 1) {
    new_tileset = convert_v1_to_v2(new_tileset)
  }
  if (new_tileset.format_version === 2) {
    new_tileset = convert_v2_to_v3(new_tileset)
  }
  if (new_tileset.format_version === 3) {
    new_tileset = await convert_v3_to_v4(new_tileset)
  }
