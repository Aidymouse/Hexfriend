<script lang="ts">
  ...
  async function exportAsHexfriend(id: number) {
    ...
    download(
      ...
      `${mapData.title ? mapData.title : 'Untitled Hexfriend'}.hexfriend`,
      'appliation/json',
    )
  }
            ...
            <div class="map-save" class:error={save.saveVersion != LATESTSAVEDATAVERSION}>
              ...
              {#if save.saveVersion != LATESTSAVEDATAVERSION}
                <div
                  ...
                  title={'This save data is on an old version - you can still load it and it will try to update itself'}
                >
                  Old version!
                </div>
              {/if}
