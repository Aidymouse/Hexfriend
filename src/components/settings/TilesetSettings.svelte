<script lang="ts">
	...
	import {
		update_map_to_new_default_tileset,
		update_tileset_format,
	} from "../../lib/tileset_updater";
	...
	function importTileset() {
		...
		r.onload = (eb) => {
			...
			let set_already_imported = loadedTilesets.find(
				(ts: Tileset) => ts.id == setToImport.id,
			);
