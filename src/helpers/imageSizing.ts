 ...
 */
export const get_image_scaled_for_hex_relative = (
  texWidth: number,
  texHeight: number,
  hexWidth: number,
  hexHeight: number,
  proportion_of_hex: number,
) => {
  let scale: number
 ...
 */
export const get_image_scaled_for_hex_bydimension = (
  texWidth: number,
  texHeight: number,
  hexWidth: number,
  hexHeight: number,
  proportion_of_hex_width: number,
  proportion_of_hex_height: number,
) => {
  return {
...
export const get_icon_scale_for_hex = (
  icon: Icon,
  hexInfo: Pick<PreviewHexInfo, 'hexWidth' | 'hexHeight'>,
): { x: number; y: number } => {
  ...
  return get_image_scaled_for_hex_relative(
    icon.texWidth,
    icon.texHeight,
    hexInfo.hexWidth,
    hexInfo.hexHeight,
