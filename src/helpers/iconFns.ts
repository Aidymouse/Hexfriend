...
import { type PreviewHexInfo } from '../types/terrain'
 ...
 * */
export async function generate_icon_preview(
  icon: Icon,
  hexInfo: PreviewHexInfo,
  grph: PIXI.Graphics,
  spr: PIXI.Sprite,
  cont: PIXI.Container,
  app: PIXI.Application,
  customTexture?: PIXI.Texture,
): Promise<string> {
  //debug && console.log("Generating icon preview for", icon.id)

  let path = getHexPath(hexInfo.width, hexInfo.height, hexInfo.orientation, 0, 0)
  grph.clear()
...
export async function generate_ordered_icon_preview(
  icon: Icon,
  hexInfo: PreviewHexInfo,
  customTexture: PIXI.Texture,
): Promise<string> {
...
export const getIconPositionFor = (
  x: number,
  y: number,
  hexInfo: Omit<PreviewHexInfo, 'color'> & { gap: number },
  snap: boolean,
): { x: number; y: number } => {
  if (!snap) {
    return { x, y }
  ...
  let clickedHexCoords = coords_worldToCube(x, y, hexInfo.orientation, hexInfo.width, hexInfo.height, hexInfo.gap)
