...
export const getPathSnapPoint = (x: number, y: number, hex_info: HexSizeInfo) => {
  ...
  let snap_grid_hexWidth =
    (hex_info.hexWidth + hex_info.gap) / (hex_info.orientation == HexOrientation.FLATTOP ? 2 : 1.5)
  let snap_grid_hexHeight =
    (hex_info.hexHeight + hex_info.gap) / (hex_info.orientation == HexOrientation.FLATTOP ? 1.5 : 2)
