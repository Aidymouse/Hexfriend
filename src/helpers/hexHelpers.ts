import type { HexGridParams, HexSizeParams, TerrainField, TerrainHex } from '../types/terrain'
...
export function get_width_height_from_radius(radius: number, orientation: HexOrientation) {
...
export function breakDownHexID(hexId: HexId) {
  ...
  return { q: parseInt(brokenId[0]), r: parseInt(brokenId[1]), s: parseInt(brokenId[2]) }
}
...
export function getHexPathRadius(
  radius: number,
  orientation: HexOrientation = HexOrientation.FLATTOP,
  centerX: number = 0,
  centerY: number = 0,
...
export const getHexSizeParams = (terrainField: TerrainField): HexSizeParams => ({
  width: terrainField.hexWidth,
  height: terrainField.hexHeight,
  orientation: terrainField.orientation,
})

export const getHexGridParams = (terrainField: TerrainField): HexGridParams => ({
  width: terrainField.hexWidth,
  height: terrainField.hexHeight,
  orientation: terrainField.orientation,
  gap: terrainField.gap,
  raised: terrainField.raised,
})
