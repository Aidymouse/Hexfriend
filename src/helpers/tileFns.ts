 ...
 * */
const debug = false
export async function generate_tile_previews(
  tile: Tile,
  preview_hex_info: PreviewHexInfo,
  spr: PIXI.Sprite,
  grph: PIXI.Graphics,
  cont: PIXI.Container,
  app: PIXI.Application,
  load_texture_from_tile: boolean = false,
) {
  ...
  if (preview_hex_info.orientation === HexOrientation.POINTYTOP) {
    preview_hex_ft.height = preview_hex_info.width
    preview_hex_ft.width = preview_hex_info.height
  }
  ...
  if (preview_hex_info.orientation === HexOrientation.FLATTOP) {
    preview_hex_pt.height = preview_hex_info.width
    preview_hex_pt.width = preview_hex_info.height
  }
...
export async function generate_tile_preview(
  tile: Tile,
  preview_hex_info: PreviewHexInfo,
  spr: PIXI.Sprite,
  grph: PIXI.Graphics,
  ...
  if (tile.symbol) {
    ...
    debug &&
      console.log('generate_tile_previews', 'resultant tex dimensions', {
        width: tile.symbol.texWidth * symbol_scale.x,
        height: tile.symbol.texHeight * symbol_scale.y,
      })
  ...
  grph.drawPolygon(getHexPath(preview_hex_info.width, preview_hex_info.height, preview_hex_info.orientation, 0, 0))
  grph.endFill()
